...
- Craft / polish (demonstrate craft)
  ...
  - **Focus / keyboard findings** (first audit, 2026-08-15, seed 1, 1422 assertions over 13 pages):
    - **Fix — `/type/` preview facsimile has a live skip link.** `scripts/font-explore/generate-font-gallery.js:97` emits `<a class="skip" href="#font-preview-main">Skip to content</a>` inside the decorative site facsimile. It is in the real tab order, sits off-canvas at rest, and on focus renders at the document's top-left — outside `div#font-preview-jp` (`overflow: hidden`), so it is clipped and never visible. A facsimile shouldn't contribute tab stops at all; `inert` on the preview is the likely fix. Fails 2.4.11 focus not obscured, and explains the one `/type/` focus-order divergence. `/color/` does not share this — the link comes from the font generator, not `scripts/utils/preview-site-lockup.js`.
    - **Fix — lightbox Next button drops focus when it disables.** At the last figure, `#figure-lightbox-next` becomes disabled while focused, and focus falls to nothing rather than moving to a sensible neighbor. Confirmed by the `lightbox-next-button` scenario. 2.4.3 focus order.
    - **Fix — content-warning `<details>` summary has `:hover` styling but no `:focus-visible`** (`jonplummer.css` 639–649). Known before this audit and **not** caught by it: the UA outline still changes pixels, so the visibility check passes. The hover-vs-focus follow-up below is what would catch it.
    - **Judgment pending — `/color/` focus order diverges from visual order at 18 transitions.** All inside the gallery preview facsimiles. Plausibly correct for side-by-side light/dark columns, but I read selectors rather than watching it, so this is unresolved rather than dismissed.
    - **Known gap — the two pages with suspected control-focus-styling defects are the two pages the audit measures least.** `/color/` and `/type/` are marked `sanityOnly` (sweep without screenshots) because their control counts would dominate the run, so the `/color/` + `/type/` form controls said to have no focus styling get no visibility evidence. Either measure a subset of stops on those pages or check them by hand.
