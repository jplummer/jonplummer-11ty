...
Prioritized by **side-effect surface** — complexity that can produce a silently wrong result, not merely large files. Source audits: **`docs/superpowers/complexity-opus5.md`**, **`complexity-grok4.6.md`**, **`complexity-composer2.5.md`** (all 2026-08-13; all three flagged both items below).
...
- **Trim test runner presentation** — Same track as the bullet above; presentation-only slice. After **`scripts/test-manifest.js`** lands, collapse **`runFastTests` / `runAllTests` / `runUnitTests`** into one **`runTestGroup(label, ids)`** loop; derive **`all`** as **`fast` + a11y** instead of a third copied array; derive **`listTests()`** primary names from the manifest instead of a fourth hardcoded list. Do **not** remove JSON IPC or **`writeSyncAll`** — Opus/composer audits agree those earned their keep. Verify: **`pnpm run test fast`**, **`pnpm run test unit`**, **`pnpm run test`** (list output).
...
- **IndexNow — simplify or remove (don't fix in place)** — Shipped 2026-01-11; ~860 lines across **`scripts/utils/indexnow.js`** + **`scripts/test/indexnow.js`**: git-diff Added/Modified filters, **`.indexnow-state.json`**, hand-rolled permalink mapping for new posts, static-page allowlist, **`https.request`** while Cloudflare purge uses **`fetch`**. Runs on every deploy + in **`test fast`**. **Value**: IndexNow is a *hint* protocol — marginal on a personal blog that already ships **`sitemap.xml`**, RSS, and normal crawl paths. **Don't invest in fixing** the bespoke git/permalink/state machine; either **(A) replace** with one post-deploy call submitting **`https://jonplummer.com/sitemap.xml`** (keep key file + env var), or **(B) remove** from deploy, test roster, and docs if indexing hints aren't worth any code. **`INDEXNOW_ENDPOINT`** was dead code; remove if still present. If choosing (B), keep **`redirects.yaml`** / SEO as-is — this isn't about discoverability of the site overall.
...
- **Libre Franklin body** (2026-08-10) — Replaced Public Sans (en-dash/space kerning bug). `@font-face` in `jonplummer.css`; `fonts.css` kept for OG extract. Spec notes in `docs/font-stack-exploration.md`.
