...
- Craft / polish (demonstrate craft)
  ...
  - **Focus / keyboard findings** (first audit, 2026-08-15, seed 1, 1422 assertions over 13 pages):
    ...
    - **Fixed 2026-08-16 — `/color/` + `/type/` form controls use a site accent outline.** Hand check showed the UA blue ring was visible (craft, not an a11y failure). `select:focus-visible` and `input[type="range"]:focus-visible` now take `outline: 2px solid var(--link-color)` in `jonplummer.css`.
