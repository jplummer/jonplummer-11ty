...
- Side project pages — two things that clear the `sides/` bar and have no page there (2026-09-04; **this site** shipped 2026-09-07, see DONE)
  - **Book map tool** — built for my wife to edit and export for her personal website; **not actually in use**, so say so in `status` rather than implying it is. The one project here that can be *previewed on its own page* — an embedded live sample or exported map beats a screenshot. She's happy to have this published. Recheck the map tile licensing so nothing breaks live.
