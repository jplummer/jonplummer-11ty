...
| **Live color gallery**    | `src/color.njk` + embed partial + scoped CSS — run `pnpm run dev` or `pnpm run build`, open `**/color/`**. Embed markup/CSS are produced during **`eleventy.before`** (same defaults as `pnpm run color-gallery` with no flags, plus **`stableWildThemes: true`** so wild-pack randomness is deterministic and **`--watch`** does not rewrite `src/_includes/partials/color-gallery-embed-inner.html` every cycle). Gallery **page chrome** uses the same **design tokens** as the rest of the site. Commit the two `src/…` embed files when you change the generator and want that diff in git / CI. |
