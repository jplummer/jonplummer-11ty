...
/** Same-origin `<script src>` so Apache CSP `script-src 'self'` allows the gallery (inline scripts are blocked). */
const SITE_COLOR_EMBED_JS = path.join(process.cwd(), 'src', 'assets', 'js', 'color-gallery-embed.js');
...
/** OKLCH hue disk + `.color-wheel-dots` markers (filled by `color-gallery-embed.js`). */
...
/**
 * Full-spectrum conic with explicit stops. Uses OKLCH hues so disk angles match marker dots and
 * preview tokens (an HSL disk misaligns ~10–20° vs OKLCH for the same nominal hue).
 */
...
/**
 * Gallery interaction script (hue wheels, combos, harmony lab).
...
<head>
  ...
  <style>
    ...
    */
    ...
    /*
      Live site is one sheet (--content-background-color on body and bands).
      Preview card chrome (border/radius/shadow) stays; field matches the reading surface.
    */
    ...
    */
    ...
    .theme-root.home-preview.revision-placement-preview .jp-page hgroup h1 a:any-link:is(:hover, :focus-visible) {
      color: var(--link-color);
    }
...
${embedHeaderHtml}${bodyContent}
  <script src="${siteEmbed ? '/assets/js/color-gallery-embed.js' : '../../../src/assets/js/color-gallery-embed.js'}"></script>
</body>
</html>`;
}
...
function writeColorGalleryEmbedFiles(fullHtml) {
  ...
  const jsHeader =
    '/* Generated by scripts/color-explore/generate-gallery.js — not hand-edited. */\n';
  const jsChanged = writeTextFileIfChanged(
    SITE_COLOR_EMBED_JS,
    `${jsHeader}${galleryEmbedRuntimeScriptJs()}\n`
  );

  return cssChanged || innerChanged || jsChanged;
}
...
function runColorGalleryBuild(opts = {}) {
  ...
  if (!quiet && embedTouched) {
    ...
    console.log(`Wrote ${SITE_COLOR_EMBED_JS}`);
  }
