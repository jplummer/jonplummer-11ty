...
The best part: this could be built in the platform today using data that already existed there, and building it would introduce patterns the platform sorely needed.
...
*Clicking any source or consumer chip opens an inspector for that system. This one shows the Salesforce integration, a complete picture of the bidirectional relationship: which fields Invoca sends to Salesforce after each call, and which fields Salesforce writes back via the Transaction Events API. That writeback configuration — disposition, product purchased, order ID — lived in Salesforce and had no visibility in the Invoca platform. Here it's visible at the field level, automatically.*
